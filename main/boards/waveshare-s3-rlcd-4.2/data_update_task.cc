...
void CustomLcdDisplay::DataUpdateTask(void *arg) {
    ...
    // 温湿度传感器刷新间隔 30 秒
    const uint32_t SENSOR_POLL_INTERVAL = 30 * 1000;
    ...
    while (1) {
        ...
        // ===== 天气更新（每 10 分钟拉取一次）=====
        const uint32_t WEATHER_FETCH_INTERVAL = 10 * 60 * 1000;  // 10 分钟
        static uint32_t last_weather_fetch_ms = 0;
        static bool weather_fetch_done = false;
        if (network_connected && idle_long_enough &&
            (last_weather_fetch_ms == 0 || (now_ms - last_weather_fetch_ms >= WEATHER_FETCH_INTERVAL))) {
            WeatherManager::getInstance().update();
            last_weather_fetch_ms = now_ms;
            if (!weather_fetch_done) weather_fetch_done = true;
        }
        ...
        if (!self->showing_system_info_) {
            ...
            if (minute_changed) {
                ...
                const char *weeks_cn[] = {"周日", "周一", "周二", "周三", "周四", "周五", "周六"};
        ...
        if (!self->showing_system_info_) {
            ...
            if (allow_noncritical_update) {
                ...
                // 2. 温湿度更新（每 30 秒采样一次）
                if (last_sensor_poll_ms == 0 || (now_ms - last_sensor_poll_ms >= SENSOR_POLL_INTERVAL)) {
                    SensorData sd = SensorManager::getInstance().getTempHumidity();
                    if (sd.valid) {
                        char buf[48];
                        snprintf(buf, sizeof(buf), "室温: %.1f°C  湿度: %.0f%%", sd.temperature, sd.humidity);
